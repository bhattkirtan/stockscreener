"""
🚀 Live Trading Bot for Capital.com - M15 (5-Minute) Timeframe
...
GOLD M15 Strategy:
- Timeframe: M15 (5-minute candles)
...
Note: M15 generates signals 3x faster than M15
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('trading_bot_m15.log'),
...
class TradingBotM15:
    ...
    def __init__(self, config: TradingConfig, epic: str = 'GOLD'):
        ...
        # Strategy configuration (GOLD M15 parameters - adjusted for faster timeframe)
        self.strategy = SupertrendVWAPStrategy(
            supertrend_period=7,
            supertrend_multiplier=2.0,
            sma_fast=21,   # Optimized for M15 (vs 21 for M15)
            sma_slow=50,   # Optimized for M15 (vs 50 for M15)
            ema_period=10,
            bb_period=20,
            bb_std=2.0,
        ...
        # Historical M15 bars for indicator calculation
        self.m15_history: List[Dict] = []
        self.min_history_bars = 20  # Need at least 21 bars for slow SMA (~100 min for M15)
        ...
        logger.info(f"🤖 M15 Trading Bot initialized: Epic={epic}, Mode={'AUTO-TRADE' if self.auto_trade else 'SIGNAL-ONLY'}, Timeframe=M15")
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info("📊 Loading historical M15 candles...")
            try:
                ...
                if historical_candles:
                    self.m15_history = historical_candles
                    count = len(self.m15_history)
                    sma_ready = count >= 21
                    duration_min = count * 15
                    logger.info(f"✅ Loaded {count} historical M15 candles ({duration_min} min)")
            ...
            # Subscribe to GOLD M15 OHLC candles
            await self.ws_client.subscribe_ohlc([self.epic], resolution='MINUTE_15')
            
            # Subscribe to live quotes for current price
            await self.ws_client.subscribe_quotes([self.epic])
            
            logger.info(f"🎯 Subscribed to {self.epic} M15 candles and live quotes")
    ...
    async def on_m5_candle(self, candle: Dict):
        """
        Callback when M15 OHLC candle is received - uses M15 directly (no aggregation needed)
        
        Args:
            candle: M15 candle data (includes 'epic', 'open', 'high', 'low', 'close', 'timestamp', 'price_type')
        """
        ...
        logger.info(f"📊 M15 Candle: {candle['timestamp']} O:{candle['open']:.2f} H:{candle['high']:.2f} L:{candle['low']:.2f} C:{candle['close']:.2f}")
        
        # Save candle to file
        self.save_candle_to_file(candle)
        
        # Use M15 candles directly (no aggregation)
        self.m15_history.append(candle)
        prev_count = len(self.m15_history)
        
        # Keep only last 50 bars (enough for indicators + some buffer)
        if len(self.m15_history) > 50:
            trimmed = len(self.m15_history) - 50
            self.m15_history = self.m15_history[-50:]
        ...
        logger.info(f"📊 History: {count} M15 bars ({count * 15} min) | SMA_slow: {'✓' if sma_slow_ready else '✗'} | Memory system: {'✓ ACTIVE' if memory_ready else '⏳ WAITING'}")
        
        # Wait for enough history
        if len(self.m15_history) < self.min_history_bars:
            logger.info(f"⏳ Building history: {len(self.m15_history)}/{self.min_history_bars} M15 bars (~{len(self.m15_history) * 5} minutes)")
            return
    ...
    async def on_quote(self, quote: Dict):
        ...
        # Update trailing stops on live price feed (not just M15 candles)
    ...
    def save_candle_to_file(self, candle: Dict):
        """
        Save M15 candle to local JSONL file for backup/replay
        ...
        try:
            ...
            filename = f"{self.epic}_M15_{date_str}.jsonl"
    ...
    async def generate_signal(self):
        """Generate trading signal from M15 indicators using strategy logic"""
        try:
            ...
            if pending['direction'] is not None:
                ...
                wait_time = len(self.m15_history) - pending['candle_idx']
...
async def main():
    ...
    # Create bot
    bot = TradingBotM15(config, epic='GOLD')
